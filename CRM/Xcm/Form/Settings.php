...
class CRM_Xcm_Form_Settings extends CRM_Core_Form {
  ...
  public function buildQuickForm() {
    ...
    $this->add('select',
                      ...
                      $phoneTypes,
                      TRUE,
                      ['class' => 'crm-select2 huge', 'placeholder' => TRUE]);
    $this->add('select',
                      'secondary_phone_type',
                      E::ts('Secondary Phone Type'),
                      $phoneTypes,
                      FALSE,
                      ['class' => 'crm-select2 huge', 'placeholder' => E::ts('Secondary phone not used')]);
    $this->add('select',
                      'tertiary_phone_type',
                      E::ts('Tertiary Phone Type'),
                      $phoneTypes,
                      FALSE,
                      ['class' => 'crm-select2 huge', 'placeholder' => E::ts('Tertiary phone not used')]);
    $this->add('select',
    ...
    $this->addElement('select',
                      ...
                      [
                        0 => E::ts('case-sensitive', ['domain' => 'de.systopia.xcm']),
    ...
    $this->addElement('select',
                      ...
                      [
                        0 => E::ts('No'),
                        1 => E::ts('Yes (beta)', ['domain' => 'de.systopia.xcm']),
                      ],
                      ['class' => 'crm-select2']);
    ...
    $this->addElement('select',
                      ...
                      ['0' => E::ts("Don't do that")] + $locationTypes,
                      ['class' => 'crm-select2']);

    $this->addElement(
      'checkbox',
      'match_contact_id',
      ts('Match contacts by contact ID', ['domain' => 'de.systopia.xcm'])
    );
    ...
    for ($i = 1; $i <= XCM_MAX_RULE_COUNT; $i++) {
      $this->addElement('select',
                        "rule_$i",
                        E::ts('Matching Rule #%1', [1 => $i, 'domain' => 'de.systopia.xcm']),
                        $this->getRuleOptions($i),
                        ['class' => 'crm-select2', 'style' => 'width: 400px']);
    }

    // add stuff for matched/created postprocessing
    foreach (['matched', 'created'] as $mode) {
      $this->addElement('select',
                        "{$mode}_add_group",
                        E::ts('Add to group'),
                        $this->getGroups(),
  ...
  protected function getRuleOptions($i) {
    // compile list
    if ($i > 1) {
      $none_option = [0 => E::ts('None, thank you')];
    }
    else {
      $none_option = [];
    }
    ...
    $rules = [];
    $option_values = civicrm_api3('OptionValue', 'get', ['option_group_id' => 'xcm_matching_rules', 'is_active' => 1]);
  ...
  protected function getLocationTypes() {
    $types = [];
    $result = civicrm_api3('LocationType', 'get', ['is_active' => 1]);
  ...
  protected function getPhoneTypes() {
    ...
    $result = civicrm_api3(
      'OptionValue',
      'get',
      ['is_active' => 1, 'option_group_id' => 'phone_type', 'option.limit' => 0]
    );
    foreach ($result['values'] as $type) {
  ...
  protected function getWebsiteTypes() {
    ...
    $result = civicrm_api3(
      'OptionValue',
      'get',
      ['is_active' => 1, 'option_group_id' => 'website_type', 'option.limit' => 0]
    );
    foreach ($result['values'] as $type) {
      $types[$type['value']] = $type['label'];
    }
  ...
  protected function getActivities($none_option = TRUE) {
    ...
    $activities = civicrm_api3(
      'OptionValue',
      'get',
      ['is_active' => 1, 'option_group_id' => 'activity_type', 'option.limit' => 0]
    );
  ...
  protected function getActivityStatuses() {
    ...
    $statuses = civicrm_api3(
      'OptionValue',
      'get',
      ['is_active' => 1, 'option_group_id' => 'activity_status', 'option.limit' => 0]
    );
  ...
  protected function getCampaigns() {
    ...
    $campaigns = civicrm_api3('Campaign', 'get', ['is_active' => 1, 'option.limit' => 0]);
  ...
  protected function getTags() {
    $tag_list = [0 => E::ts('None, thank you')];

    $tags = civicrm_api3('Tag', 'get', ['is_active' => 1, 'option.limit' => 0]);
  ...
  protected function getTemplates() {
    $template_list = [0 => E::ts('No content')];

    $templates = civicrm_api3('MessageTemplate', 'get', ['is_active' => 1, 'is_reserved' => 0, 'option.limit' => 0]);
  ...
  protected function getGroups() {
    $group_list = [0 => E::ts('None, thank you')];

    $groups = civicrm_api3('Group', 'get', ['is_active' => 1, 'option.limit' => 0]);
